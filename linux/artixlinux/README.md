...
### Mount Internal NTFS SATA SSDs for TRIM support

This is a sightly niche use case, but applicable to people who used to have Windows and don't want to migrate all of their data NTFS drives to ext4. 

By default, Artix doesn't come with a tool to automatically insert fstab entries. KDE provide a GUI auto-mount functionality, but there is no configurability. I also found that `discard` is not enabled using that option, so we might not have TRIM support at all.

To resolve that:
- I disabled KDE auto-mount for these drives.
...
- In my case, I need to symlink them to `/run/media/hoangbui`, because my other programs are expecting this path. So I add a script to `/etc/local.d`, like so
```
sudo nano /etc/local.d/hoangbui-mount-compat.start
```
```
mkdir -p /run/media/hoangbui
ln -s /mnt/Programs /run/media/hoangbui/Programs
ln -s /mnt/Data /run/media/hoangbui/Data
ln -s /mnt/Work /run/media/hoangbui/Work
```
Then make it executable
```
sudo chmod +x /etc/local.d/hoangbui-mount-compat.start
```
This will run after the drives are mounted and make sure my programs still work. In the future if I bother to fix the paths in the config of those programs, I can then remove this script.
- It is possible that `localmount` is not even running by default, so add it
```
